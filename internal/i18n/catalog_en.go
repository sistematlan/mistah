...
var english = map[string]string{
	...
	// ---- Dev (old Xcode simulators) ----
	"dev.xcode-simulator.detail.simple":   "Simulator %s on runtime %s (no longer available)",
	"dev.xcode-simulator.detail.advanced": "CoreSimulator device %s, runtime %s unavailable",
	...
	"wizard.menu.dev-detected":   "Detected dev tools — your developer caches are included too.",
	"wizard.cancelled":           "Cancelled. Nothing was deleted.",
	"wizard.thanks":              "Done. Thanks for using mistah.",
	"wizard.level.light.name":    "Light",
	"wizard.level.light.desc":    "App caches, Trash, thumbnails — what every Mac piles up",
	"wizard.level.standard.name": "Standard",
	"wizard.level.standard.desc": "Light + Docker, Xcode, Time Machine snapshots, logs",
	"wizard.level.deep.name":     "Deep",
	"wizard.level.deep.desc":     "Standard + orphans, Downloads, iPhone backups (asks per item)",
	"wizard.confirm.level":       "Level",
	"wizard.confirm.items":       "items",
	"wizard.confirm.prompt":      "Proceed? [y/N] ",
	"wizard.review.header":       "Now let's review the files that might be yours, one by one.",
	"wizard.review.subtitle":     "These may hold your data: confirm each one with y/N (v=view contents, q=quit).",
}
